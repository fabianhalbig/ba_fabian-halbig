...
Insgesamt muss für diese Packages aus der Spring-Boot ein Docker-Image erstellt werden:
- eureka-server
- api-gateway
- costumer-service
- user-service
...
2. Erstellung der PVC, der Storageklassen sowie der Postgres-Datenbankdienste:
    ...
    - `kubectl apply -f course-pv.yml`
    - `kubectl apply -f status-pv.yml`
    - `kubectl apply -f keycloak-pv.yml`
    - `kubectl apply -f course-db.yml`
    - `kubectl apply -f status-pv.yml`
...
4. Bereitstellung der Spring-Services
    ...
    - `kubectl apply -f course-service.yml`
    - `kubectl apply -f user-server.yml`
